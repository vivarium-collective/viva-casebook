    ...
    order.forEach(function (id) {
      ...
      fetch('/api/composite-resolve?id=' + encodeURIComponent(entry.id) + '&overrides=' + encodeURIComponent(entry.overridesJson))
        ...
        .then(function (res) {
          ...
          // Model tab: auto-expand the loom so each composite shows as a full
          // inline bigraph-loom explorer by default. The shared card renderer
          // (static/composite-card.js) starts collapsed for the Modules grid and
          // walkthrough, where a click-to-Explore is right; but the Model tab IS
          // the "look at this study's model" view, so open the loom eagerly.
          if (cardEl && typeof window._toggleLoomCard === 'function') {
            var exploreBtn = cardEl.querySelector('.pcard-explore-btn');
            var exploreSec = cardEl.querySelector('.pcard-sec-explore');
            if (exploreBtn && exploreSec && !exploreSec.classList.contains('pcard-sec-open')) {
              window._toggleLoomCard(exploreBtn);
            }
          }
        })
        .catch(function () {
