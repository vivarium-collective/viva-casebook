...
> 🔀 And task #4 — [**Bistability: a structural property no knob tunes into being**](docs/bistable-agent-vs-policy.html): a toggle switch is monostable without *cooperative* feedback; the policy tunes expression and stays monostable, the LLM recognises bistability needs Hill n≥2 and opens two basins in one edit.
> 🩺 And task #3 — [**Ambiguous diagnosis: read the observables, not the margin**](docs/diagnosis-agent-vs-policy.html): biomass is low because the cell is *dying*; the policy misdiagnoses off the worst margin (boost yield) and gives up, the LLM reads the crashed viability and installs the one correct fix.
> 🧬 And task #7 — [**SBML → COPASI: the agent must author a model**](docs/sbml-agent-vs-policy.html): the agent authors an SBML reaction network, loaded + simulated in the REAL COPASI backend (basico/viva-copasi); the policy can't author SBML structure and gives up at 0/5, the LLM builds A→B→C to 5/5.
